    ...
    public String savePerson(Model model, @Valid Person person, Errors errors, @RequestParam("photoFilename") MultipartFile photoFile) throws IOException {
        ...
        if (!errors.hasErrors()) {
            try {
                ...
                return "redirect:people";//word redirect is  to save an entry and redirect with updated data
            } catch (StorageException e) {
                model.addAttribute("errorMsg", "System i currently unable to accept");
                return "people";
            }
        }
        return "people";
